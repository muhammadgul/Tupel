...
//   double cut_de[]={0,20,40,60,80,100,120,140,160,180,200};
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(lepton_id.size()>=2 && (lepton_id[0]*lepton_id[1])<0 ){//dilepton

        TLorentzVector w1=(lepton_vector[0]+nu_vector[0]);
        TLorentzVector w2=(lepton_vector[1]+nu_vector[1]);
...
*/
       ...
       if(b_jet_vector.size()>=2 &&(b_jet_id[0]*b_jet_id[1])<0 ){ 
        // cout<<b_jet_id[0]<<" "<<b_jet_id[1]<<endl;
         ...
         if (lepton_id[0]>0){
          ...
          if(light_jet_vector.size()<1)continue;
         // cout<<"Top st3 mass, atop st3 mass, top st1 reco mass, atop st1 reco mass: "<<top_tmp.M()<<" , "<<atop_tmp.M()<<" , "<<top_vector.M()<<" , "<<atop_vector.M()<<endl;
          ...
          TLorentzVector ttj_vector=top_vector+atop_vector+light_jet_vector[0];
          ...
          double Jettheta_before=2*atan(exp(-1*light_jet_vector[0].Eta()));
          ...
          TLorentzVector light_jet_vector_ttbar=light_jet_vector[0];
	  ...
	  atop_vector.Boost(-ttj_vector.BoostVector());
          light_jet_vector[0].Boost(-ttj_vector.BoostVector());
...
*/
          ...
          double Jettheta_after=2*atan(exp(-1*light_jet_vector[0].Eta()));
          ...
          if(fabs(light_jet_vector[0].Rapidity())<0.5){//Asymetry wrt different cuts
            for(int i = 0; i<3;i++){//ttj rapidity
              for(int j = 0; j<3;j++){//De
