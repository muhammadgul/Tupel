...
TTree *tmc = (TTree *)mc->Get("tree");
Long64_t wmc;

tmc->SetBranchAddress("nentries",&wmc);
tmc->GetEntry(0);
...
cout<<wmc<<"  "<<wst_t_t<<"  "<<wst_t_tbar<<"  "<<wwjet<<endl;

hmc->Scale(831.76*5.7/wmc);
...
TH1* stackmc= (TH1D*)hmc->Clone("stackmc");
stackmc->Add(hst_t_t);
...
TH1* stacst= (TH1D*)hst_t_t->Clone("stackmc");
